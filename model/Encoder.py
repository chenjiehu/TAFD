...
class encoder_fuse10(nn.Module):
    def __init__(self, layers = 18,classes = 6, \
        ...
        self.decoder = Classifier_Module(64, [6, 12, 18, 24], [6, 12, 18, 24], 64)
...
class encoder_fuse12(nn.Module):
    ...
    def __init__(self, layers = 50, classes = 6, \
        ...
        self.encoder = Encoder(init_path=pretrained_path)
        self.decoder = Classifier_Module(256+512, [6, 12, 18, 24], [6, 12, 18, 24], 64)
...
class encoder_fuse14(nn.Module):

    def __init__(self, layers = 50, classes = 6, \
                 criterion=nn.CrossEntropyLoss(ignore_index=255), BatchNorm=nn.BatchNorm2d, \
                 pretrained_path=INIT_PATH, sync_bn=True, shot=1, ppm_scales=[60, 30, 15, 8],
                 ):
        super(encoder_fuse14, self).__init__()
        self.encoder = Encoder(init_path=pretrained_path)
        self.decoder = Classifier_Module(256+512+1024, [6, 12, 18, 24], [6, 12, 18, 24], 64)
        print('encoder_fuse8:',encoder_fuse14)
    ...
    def forward(self, x):
        ...
        feat_2_copy = F.interpolate(feat_2, size=(feat_1.shape[2], feat_1.shape[3]), mode='bilinear',align_corners=True)
        feat_3_copy = F.interpolate(feat_3, size=(feat_1.shape[2], feat_1.shape[3]), mode='bilinear',align_corners=True)
        feat = torch.cat([feat_3_copy, feat_2_copy, feat_1], 1)   # fuse other features? cjh
        feat = self.decoder(feat)
